...
| `deploy/quadlet/sre-tab-status.container` | hourly `sre-tab status` | **outstanding** — this unit is not in this branch. It arrives with PR #19 (`deploy/status-alerting`) still naming `sre-tab-database-url`. `sre-tab status` is read-only — `refresh_status` and `nonconforming_slugs` are both `SELECT`s — so it belongs on **`sretab_readonly`**: `Secret=sre-tab-readonly-password` is the wrong shape for it (the CLI wants a whole `DATABASE_URL`), so closing it means minting a `sre-tab-readonly-database-url` alongside the bare password, or giving the unit `sretab_app` and accepting a wider role than it needs. Whichever branch merges second owns this row |
